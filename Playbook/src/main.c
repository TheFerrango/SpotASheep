...
void add_sheep() {
    ...
    if(rand()%8 == 0)
    	boxes[num_boxes].color = 250.0 ;
    else
    	boxes[num_boxes].color = 0.0 ;
    boxes[num_boxes].x = width+(rand() %200);
    boxes[num_boxes].y = ((float) (rand() % 8))*100.0;;
    boxes[num_boxes].width = 200;
    boxes[num_boxes].height = 50;
...
void render() {
    ...
    for (i = 0; i < num_boxes; i++) {
        ...
        glScalef(boxes[i].width, boxes[i].height, 1.0f);
...
static void handleSensorEvent(bps_event_t *event) {
    if (SENSOR_AZIMUTH_PITCH_ROLL_READING == bps_event_get_code(event)) {
        ...
        float result_x = -1.0f, result_y = 0.0f;
